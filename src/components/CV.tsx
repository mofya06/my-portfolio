      ...
      <section id="cv" className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className="text-4xl md:text-5xl font-bold text-slate-800 mb-6">Curriculum Vitae</h2>
          ...
          {/* Contact Information */}
          <div className="mb-16 bg-slate-50 rounded-xl p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              {contactInfo.map((info) => (
                  <div key={info.label} className="flex items-center gap-3 justify-center md:justify-start">
                    ...
                    <div>
                      <p className="font-medium text-slate-800 text-sm">{info.label}</p>
                      {info.href ? (
                          <a
                              href={info.href}
          ...
          <div className="mb-16">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
              {skills.map((skill) => (
                  <div key={skill} className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-lg p-3 text-center hover:shadow-md transition-all duration-300">
          ...
          <div className="mb-16">
            ...
            <div className="space-y-8">
              {experience.map((job) => (
                  <div key={job.title} className="bg-slate-50 rounded-xl p-6 hover:shadow-lg transition-all duration-300">
                    ...
                    <div>
                      ...
                      <ul className="list-disc list-inside space-y-1">
                        {job.achievements.map((achievement) => (
                            <li key={achievement} className="text-slate-600">{achievement}</li>
                        ))}
                      </ul>
                    </div>
                  </div>
          ...
          <div className="mb-16">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {education.map((edu) => (
                  <div key={edu.degree} className="bg-slate-50 rounded-xl p-6 hover:shadow-lg transition-all duration-300">
                    ...
                    {edu.gpa && <p className="text-slate-600 mb-3">GPA: {edu.gpa}</p>}
                    <div>
                      <h5 className="font-medium text-slate-800 mb-2">Relevant Coursework:</h5>
                      <div className="flex flex-wrap gap-2">
                        {edu.relevant.map((course) => (
                            <span key={course} className="bg-blue-100 text-blue-800 px-2 py-1 rounded text-xs">
                        {course}
                      </span>
                        ))}
                      </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {certifications.map((cert) => (
                  <div key={cert} className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-lg p-4 hover:shadow-md transition-all duration-300">
